...
async def read_targets_csv(session, csv_url, max_rows=50):
    ...
    for i, row in enumerate(reader):
        if i >= max_rows: break
        try:
            rid, source, company, title, url, location, fitscore = row[0], row[1], row[2], row[3], row[4], row[5], row[6]
        except Exception:
            continue
        rows.append({
            "id": rid, "source": source, "company": company, "title": title,
...
async def resolve_from_jobpage(session, job_url):
    ...
    issues = ""
    if not homepage: issues = (issues + ";no_homepage").lstrip(";")
    if not li:       issues = (issues + ";no_linkedin").lstrip(";")

    return {"ResolvedCompanyURL": homepage, "LinkedInURL": li, "Issues": issues}

async def post_qa_results(session, sheet_webapp_url, rows):
    payload = {"mode":"qa", "rows": rows, "ts": int(time.time())}
    try:
        async with session.post(sheet_webapp_url, json=payload, timeout=TIMEOUT) as r:
            txt = await r.text()
            print("POST QA → Sheets:", r.status, txt[:300])
            return r.status == 200
    except Exception as e:
        print("POST QA failed:", repr(e))
        return False
...
async def main():
    ...
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        for t in targets:
            job_url = t.get("url","")
            rid     = t.get("id","")
            if not job_url or not rid: continue
        ...
        if results:
            ok = await post_qa_results(session, sheet_ep, results)
            if not ok:
                raise SystemExit(3)
